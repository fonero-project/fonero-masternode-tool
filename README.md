# Fonero Masternode Tool (DMT)

## Contents

 * [Masternodes](#masternodes)
 * [Fonero Masternode Tool](#fonero-masternode-tool)
 ...
 * [Configuration](#configuration)
   ...
   * [Masternode setup](#masternode-setup)
     * [Scenario A: moving masternode management from Fonero Core](doc/config-masternodes-a.md)
 ...
 * [Downloads](https://github.com/fonero-project/fonero-masternode-tool/releases/latest)
 * [Changelog](changelog.md)

## Masternodes

Fonero masternodes are full nodes which are incentivized by receiving a share of the block reward as payment in return for the tasks they perform for the network, of which the most important include participation in *InstantSend* and *PrivateSend* transactions. In order to run a masternode, apart from setting up a server which runs the software, you must dedicate 50000 Fonero as *collateral*, which is *"tied up"* in your node as long as you want it to be considered a masternode by the network. It is worth mentioning that the private key controlling the funds can (and for security reasons, should) be kept separately from the masternode server itself.

A server with the Fonero daemon software installed will operate as a Fonero full node, but before the rest of the network accepts it as a legitimate masternode, one more thing must happen: the person controlling the node must prove that they are also in control of the private key to the node's 50000 Fonero *collateral*. This is achieved by sending a special message to the network (`start masternode` message), signed by this private key.

This action can be carried out using the *Fonero Core* reference software client. As can be expected, this requires sending 50000 Fonero to an address controlled by the *Fonero Core* wallet. After the recent increase in the value of Fonero and a burst in the amount of malware distributed over the Internet, you do not have to be paranoid to conclude that keeping large amounts of funds in a software wallet is not the most secure option. For these reasons, it is highly recommended to use a **hardware wallet** for this purpose.

# Fonero Masternode Tool
...
* Transferring masternode earnings safely, without touching the 50000 Fonero funding transaction
...
Most of the application features involve exchanging data between the application itself and the Fonero network. To do this, *DMT* needs to connect to one of the full nodes on the network, specifically one which can handle JSON-RPC requests. This node plays the role of a gateway for *DMT* to the Fonero network. It does not matter which full node node provides the service, because all nodes reach consensus by synchronizing information between each other on the Fonero network.

Depending on your preferences (and skills) you can choose one of three possible connection types:
 * [Direct connection to a local node](doc/config-connection-direct.md), for example to *Fonero Core* running on your normal computer.
 * [Connection to a remote node through an SSH tunnel](doc/config-connection-ssh.md), if you want to work with a remote Fonero daemon (like your masternode) through an SSH tunnel.
...
Here we make the following assumptions:
  * You already have a server running the Fonero daemon software (*fonerod*) that you want to use as a masternode. If you don't, you will need to install and configure one first by following the guide on the [Dash Wiki](https://dashpay.atlassian.net/wiki/spaces/DOC/pages/113934340).
  * We occasionally refer to the *fonerod* configuration file, so it is assumed that *fonerod* is running under a Linux operating system (OS), which is the most popular and recommended OS for this purpose.
  * Your server has a public IP address that will be visible on the Internet.
  * You have set up a TCP port on which your *fonerod* listens for incoming connections (usually 9999).

Further configuration steps depend on whether you already have a masternode controlled by *Fonero Core* which you want to migrate to a hardware wallet managed by *DMT*, or if you are setting up a new masternode.
...
* `--config`: a non-standard path to a configuration file. Example:
  `FoneroMasternodeTool.exe --config=C:\dmt-configs\config1.ini`
...
Once you set up the Fonero daemon and perform the required *DMT* configuration, you need to broadcast the `start masternode` message to the Fonero network, so the other Fonero nodes recognize your daemon as a masternode and add it to the payment queue.
...
2. Opening a connection to the Fonero network and verifying if the Fonero daemon to which it is connected is not still waiting for synchronization to complete.
  Message in case of failure:  
  ![Fonero daemon synchronizing](doc/img/startmn-synchronize-warning.png)
...
6. Retrieving the Fonero address from the hardware wallet for the `BIP32 path` specified in the configuration. If it differs from the collateral address provided in the configuration, the following warning appears:  
  ![Fonero address mismatch](doc/img/startmn-addr-mismatch-warning.png)  
  The most common reason for this error is mistyping the hardware wallet passphrase. Remember that different passphrases result in different Fonero addresses for the same BIP32 path.

7. Verification that the specified transaction ID exists, points to your collateral address, is unspent and is equal to exactly 50000 Fonero. Messages in case of failure:  
  ![Could not find the specified transaction id](doc/img/startmn-tx-warning.png)  
  ![Collateral transaction output should equal 50000 Fonero](doc/img/startmn-collateral-warning.png)  
  If you decide to continue anyway, you probably won't be able to successfully start your masternode.

8. Verification at the Fonero network level that the specified transaction ID is valid. Message in case of failure:  
...
DMT version 0.9.4 and above allows you to transfer your masternode earnings. Unlike other Fonero wallets, DMT gives you a 100% control over which *unspent transaction outputs* (utxo) you want to transfer. This has the same effect as the `Coin control` functionality implemented in the *Fonero Core* wallet.

The `Transfer funds` window shows all *UTXOs* of the currently selected Masternode (mode 1), all Masternodes in current configuration (mode 2) or any address controlled by a hardware wallet (mode 3). All *UTXOs* not used as collateral are initially selected. All collateral *UTXOs* (50000 Fonero) are initially hidden to avoid unintentionally spending collateral funds and thus breaking MN. You can show these hidden entries by unchecking the `Hide collateral utxos` option.
...
After signing the transaction with your hardware wallet, the application will display a summary and will ask you for confirmation for broadcasting the signed transaction to the Fonero network.  
![Broadcast signed transaction confirmation](doc/img/dmt-transfer-funds-broadcast.png)

After clicking `Send Transaction`, the application broadcasts the transaction and then shows a confirmation with a transaction ID as a hyperlink directing to a Fonero block explorer:  
...
Binary versions of the latest release can be downloaded from: https://github.com/fonero-project/fonero-masternode-tool/releases/latest.
...
2. Change the current directory to the folder where the DMT release files were downloaded:
   ...
   ```
   <username>: ls
   FoneroMasternodeTool_0.9.15.mac.zip    FoneroMasternodeTool_0.9.15.mac.zip.asc
   ```
...
3. Verify the signature by executing the following command:
   ...
   ```
   keybase pgp verify -d FoneroMasternodeTool_0.9.15.mac.zip.asc -i FoneroMasternodeTool_0.9.15.mac.zip -S bertrand256
   ```
